...
function PaperItem({
  ...
  const t = useT();
  const { translated: title } = useTranslate(paper.title);
  const { translated: abstract } = useTranslate(paper.abstract);

  return (
    <motion.div
      initial={{ opacity: 0, y: 14 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.4, delay: Math.min(index * 0.06, 0.5), ease: [0.16, 1, 0.3, 1] }}
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 space-y-1.5 w-full">
          <div className="flex items-start justify-between gap-2">
            <h4
              className={`text-xs font-semibold leading-snug text-[#ECEFF4] transition-colors group-hover:text-white ${
          ...
          <div
            className={`text-[11px] leading-relaxed text-[#9aa7bd] font-medium ${
              isExpanded ? "" : "line-clamp-3"
            }`}
          >
            <MarkdownText text={abstract} />
          </div>
